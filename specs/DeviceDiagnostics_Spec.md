...
| `RebootInfo` | `{timestamp, source, reason, customReason, otherReason, lastHardPowerReset}` — defined but not yet exposed via JSON-RPC |
...
- **OQ-01 — `RebootInfo` struct:** `RebootInfo` is defined in `IDeviceDiagnostics.h` with fields `timestamp`, `source`, `reason`, `customReason`, `otherReason`, and `lastHardPowerReset`, but no JSON-RPC method currently exposes it. Is there a planned `getRebootInfo` or `getLastRebootReason` method? Should this be tracked as a future requirement?
